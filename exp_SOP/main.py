...
optimizer, scheduler = setOptimizer(model.parameters(), args.lr, [int(Epochs*0.5), int(Epochs*0.75)])
...
for epoch in range(Epochs+1): 
    ...
    if epoch>0:  
        ...
        if Data in ['SOP','ICR','HOTEL']:
            sampler = BalanceSampler_sample(dsets.intervals, GSize=N_size, repeat=Repeat)
        else:
            sampler = BalanceSampler_filled(dsets.intervals, GSize=N_size, repeat=Repeat)
